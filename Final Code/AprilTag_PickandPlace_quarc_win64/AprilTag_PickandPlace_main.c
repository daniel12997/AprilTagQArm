...
typedef RT_MODEL_AprilTag_PickandPlac_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * AprilTag_PickandPlace(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(AprilTag_PickandPlace_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(AprilTag_PickandPlace_DW.HILReadTimebase1_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: AprilTag_PickandPlace -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(AprilTag_PickandPlace_DW.HILReadTimebase1_Task,
          (t_clock) AprilTag_PickandPlace_P.HILReadTimebase1_Clock, 500.0, -1)
          ;
        if (result == 0) {
